...
```{r}
#CT severity score plots

data$CTSS_score_1<- as.numeric(as.character(data$CTSS_score_1))
data$CTSS_score_2<- as.numeric(as.character(data$CTSS_score_2))

CT1.a <- ggplot(data = data, 
       mapping = aes(x = CTSS_score_1,
                     y = Hospitalization_days))+
...
CT1.b <- ggplot(data = data, 
       mapping = aes(x = CTSS_score_2,
                     y = Hospitalization_days))+
...
```{r}
#CTSS score leads to death?

ggplot(data = data |> 
        group_by(Death) |> 
        mutate(
    CTSS_score_1 = as.numeric(as.character(CTSS_score_1)),
    CTSS_score_2 = as.numeric(as.character(CTSS_score_2))
  ) |> 
        summarize(mean_CTSS_1 = mean(CTSS_score_1, na.rm = TRUE)),
       mapping = aes(x = Death, 
                     y = mean_CTSS_1,
                     fill = Death))+
  ...
  theme_minimal() +
  labs(
    title = "Mean CTSS_1 by Mortality",
    x = "Mortality (0 = Survived, 1 = Died)",
    y = "Mean CTSS Score"
  ) +
...
ggplot(data = data |> 
    ...
    CTSS_score_2 = as.numeric(as.character(CTSS_score_2))
        ...
        summarize(mean_CTSS_2 = mean(CTSS_score_2, na.rm = TRUE)),
       mapping = aes(x = Death, 
                     y = mean_CTSS_2,
                     fill = Death))+
   geom_bar(stat = "identity", width = 0.3, color = "black") +
  theme_minimal() +
  labs(
    title = "Mean CTSS_2 by Mortality",
    x = "Mortality (0 = Survived, 1 = Died)",
    y = "Mean CTSS Score"
  ) +
  scale_fill_manual(values = c("skyblue", "red"))
