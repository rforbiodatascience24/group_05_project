...
data <- data |> 
 ...
 "Antiobiotics" = "Antiobiotics (1 Yes 2 No)",
...
data_final <- data_clean3 |> 
  ...
  mutate(`Immunotherapy` = case_when(
    ...
    TRUE ~ NA_character_
  )) |> mutate(`Hormone dosage[mg]` = as.numeric(gsub("[^0-9\\.]", "", `Hormone dosage[mg]`))) |> mutate(`ImmunotheDoes the imaging improve by more than 25% after one week (1 yes 2 no)
` = case_when(
    `Does the imaging improve by more than 25% after one week (1 yes 2 no)
` == 1 ~ "yes",
    `Does the imaging improve by more than 25% after one week (1 yes 2 no)
` == 0 ~ "no",
    TRUE ~ NA_character_
  ...
  )) |> mutate(`Age ranges` = fct_relevel(`Age ranges`, "<60", "60-70", "70-80", "80-90", ">90"))
...
```{r}
data_final |>  mutate(`ICU hospitalization time[days]` = as.numeric(gsub("[^0-9\\.]", "", `ICU hospitalization time[days]`))) |> 
  select("ICU hospitalization time[days]")
```
