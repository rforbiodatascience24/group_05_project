...
process_diseases <- function(data, column, rename_map) {
  ...
  data_processed <- data |> 
    ...
    select(-"temp_prefix_NA", -"temp_prefix_0") |> 
    rename_with(
...
data_clean3 <- process_diseases(data_clean, Basic_diseases, rename_map)
  
```

```{r}
data_clean3 <- data_clean |>
  mutate(Basic_diseases = str_replace_all(Basic_diseases, "([^a-zA-Z]*).*", "\\1"))
```
