...
data |> 
  ...
  scale_fill_manual(
    values = c("Deceased" = "#5995ED", "Survived" = "#DB3069"),
  ) +
theme_minimal() +
...
diseases <- data |>
  ...
  filter(Presence == 1) |>
  select(Age_range, Outcome, Disease, Presence, Antiviral_Drug) 
...
diseases |>
  filter(!is.na(Antiviral_Drug)) |>
  summarise(n = n()) |>
  ggplot(data = diseases |>   filter(!is.na(Antiviral_Drug)) |> filter(Oxygen_Therapy == 'Mask_Oxygen_therapy'), mapping = aes(fill = Antiviral_Drug, y = n, x = Outcome)) +
  geom_bar(stat = "identity", position = "stack") +
...
data |>
  ...
  group_by(Outcome, `Disease_onset_D1_highest_body_temperature[℃]`) |>
  select(Outcome, `Disease_onset_D1_highest_body_temperature[℃]`) |>
  filter(!is.na(`Disease_onset_D1_highest_body_temperature[℃]`)) |>
ggplot(mapping = aes(x = `Disease_onset_D1_highest_body_temperature[℃]`, fill = Outcome)) +
  geom_density(alpha = 0.7) +
  scale_fill_manual(
    values = c("Deceased" = "#5995ED", "Survived" = "#DB3069"),
  ) +
